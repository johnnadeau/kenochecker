class Game < ActiveRecord::Base
  ...
  def self.load_for_date(date = Date.today)
    raw = HTTParty.get(api_url(date), format: :json)
    raw['draws'].each do |draw|
      create_from_json(draw) if game_date(draw) == date
    end
  end
  ...
  def self.create_from_json(json)
    unless exists?(game_number: json['draw_id']) 
